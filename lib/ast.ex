defmodule Fern.AST do
  ...
  defp build(f, line, stack, ret, out, cnt) do
    case stack do
      [] ->
        if ret do
          next(f, line, stack, nil, [ret | out], cnt)
        else
          f
          |> read_unit()
          |> case do
            ...
            {"", nlines} ->
              next(f, line + nlines, stack, ret, out, cnt)
            {str, nlines} ->
              next(f, line + nlines, [{:assign, str, []}], ret, out, cnt)
          end
        end
      [action | _] ->
        case action do
          ...
          {:var, ""} ->
            next(f, line, tl(stack), :null, out, cnt)
          {:var, _} = ret ->
            # case String.split(x, ".") do
            #   [var, type] ->
            #     next(f, line, tl(stack), {:get, tokenize_type(type), {:var, var}, {:const, "0"}}, out, cnt)
            #   [_] ->
            #     next(f, line, tl(stack), ret, out, cnt)
            # end
            next(f, line, tl(stack), ret, out, cnt)
          {:body, [], xs} ->
            next(f, line, tl(stack), {:body, (xs)}, out, cnt)
          {:body, [st | sts], xs} ->
            if ret do
              next(f, line, [{:body, sts, [ret | xs]} | tl(stack)], nil, out, cnt)
            else
              next(f, line, [{:assign, st, []} | stack], ret, out, cnt)
            end
          {:body, str, xs} ->
            [_ | stack] = stack
            case get_statement(str) do
              {x, rst} ->
                next(f, line, [{:body, rst, [x | xs]} | stack], ret, out, cnt)
              x ->
                next(f, line, [{:body, [x | xs], []} | stack], ret, out, cnt)
            end
          {:head, [], [a, b]} ->
            xs = case a do
              {:apply, x, xs} ->
            ...
            next(f, line, tl(stack), {:head, xs, b}, out, cnt)
          {:head, [a], xs} ->
            if ret do
              next(f, line, [{:head, [], [ret | xs]} | tl(stack)], nil, out, cnt)
            else
              next(f, line, [{:apply, a, []} | stack], ret, out, cnt)
            end
          {:head, [b, a], xs} ->
            if ret do
              next(f, line, [{:head, [a], [ret | xs]} | tl(stack)], nil, out, cnt)
            else
              next(f, line, [{:or, b, []} | stack], ret, out, cnt)
            end
          {:head, str, xs} ->
            [_ | stack] = stack
            case get_sec(str, ';') do
              {x, _, rst} ->
                next(f, line, [{:head, rst, [x | xs]} | stack], ret, out, cnt)
              x ->
                case [x | xs] do
                  [x] ->
                    next(f, line, [{:apply, x, []} | stack], ret, out, cnt)
                  xs ->
                    next(f, line, [{:head, xs, []} | stack], ret, out, cnt)
                end
            end
          {:case, [], [a, b]} ->
            a = case a do
            ...
            next(f, line, tl(stack), {:case, a, b}, out, cnt)
          {:case, [a], xs} ->
            if ret do
              next(f, line, [{:case, [], [ret | xs]} | tl(stack)], nil, out, cnt)
            else
              next(f, line, [{:head, a, []} | stack], ret, out, cnt)
            end
          {:case, [b, a], xs} ->
            if ret do
              next(f, line, [{:case, [a], [ret | xs]} | tl(stack)], nil, out, cnt)
            else
              next(f, line, [{:body, b, []} | stack], ret, out, cnt)
            end
          {:case, str, xs} ->
            [_ | stack] = stack
            case get_part(str, ["->"]) do
              {x, _, rst} ->
                next(f, line, [{:case, rst, [x | xs]} | stack], ret, out, cnt)
              x ->
                case [x | xs] do
                  [x] ->
                    next(f, line, [{:body, x, []} | stack], ret, out, cnt)
                  xs ->
                    next(f, line, [{:case, xs, []} | stack], ret, out, cnt)
                end
            end
          {:func, [], xs} ->
            next(f, line, tl(stack), {:func, xs}, out, cnt)
          {:func, [c | cs], xs} ->
            if ret do
              next(f, line, [{:func, cs, [ret | xs]} | tl(stack)], nil, out, cnt)
            else
              next(f, line, [{:case, c, []} | stack], ret, out, cnt)
            end
          {:func, str, xs} ->
            [_ | stack] = stack
            #             just to be safe
            case get_case(" " <> str) do
              {x, rst} ->
                next(f, line, [{:func, rst, [x | xs]} | stack], ret, out, cnt)
              x ->
                next(f, line, [{:func, [x | xs], []} | stack], ret, out, cnt)
            end
          {:term, x} -> # can materialize to a variety of things
            ...
            case x do
              "(" <> x -> # subexpression
                ...
                next(f, line, [{:or, x, []} | stack], ret, out, cnt)
              "{" <> x -> # function
                "}" <> x = String.reverse(x)
                x = String.reverse(x)
                next(f, line, [{:func, String.trim(x), []} | stack], ret, out, cnt)
              <<n>> <> _ = x when ?0 <= n and n <= ?9 or n == ?. -> # constant
                # make sure to wrap the constant later (compile step)
                next(f, line, [{:const, x} | stack], ret, out, cnt)
              x -> # variable
                next(f, line, [{:var, x} | stack], ret, out, cnt)
            end
          {:unary, [], [x], ops} ->
            next(f, line, [{:unary, [], x, ops} | tl(stack)], ret, out, cnt)
          {:unary, [], x, []} ->
            next(f, line, tl(stack), x, out, cnt)
          {:unary, [], x, [op | ops]} ->
            x = case op do
          ...
          {:unary, [term], [], ops} ->
            if ret do
              next(f, line, [{:unary, [], [ret], ops} | tl(stack)], nil, out, cnt)
            else
              next(f, line, [{:term, term} | stack], ret, out, cnt)
            end
          {:unary, str, ops} ->
            [_ | stack] = stack
            case str do
              "+" <> x ->
                next(f, line, [{:unary, x, ["+" | ops]} | stack], ret, out, cnt)
              "-" <> x ->
                next(f, line, [{:unary, x, ["-" | ops]} | stack], ret, out, cnt)
              "/" <> x ->
                next(f, line, [{:unary, x, ["/" | ops]} | stack], ret, out, cnt)
              "b/" <> x ->
                next(f, line, [{:unary, x, ["b/" | ops]} | stack], ret, out, cnt)
              "&" <> x ->
                next(f, line, [{:unary, x, ["&" | ops]} | stack], ret, out, cnt)
              "^" <> x ->
                next(f, line, [{:unary, x, ["^" | ops]} | stack], ret, out, cnt)
                ...
                |> case do
                  [type, xx] ->
                    if !(String.contains?(type, "(") || String.contains?(type, ")") || String.contains?(type, "{") || String.contains?(type, "}")) do
                      next(f, line, [{:unary, [String.reverse(xx)], [], ["." <> type | ops]} | stack], ret, out, cnt)
                    else
                      case ops do
                        [] ->
                          next(f, line, [{:term, x} | stack], ret, out, cnt)
                        _ ->
                          next(f, line, [{:unary, [x], [], ops} | stack], ret, out, cnt)
                      end
                    end
          ...
          {:apply, [unariable | unariables], xs} ->
            if ret do
              ...
              next(f, line, [{:unary, unariable, []} | stack], ret, out, cnt)
            end
          {:apply, str, xs} ->
            [_ | stack] = stack
            case get_unariable(str) do
              {x, rst} ->
                next(f, line, [{:apply, rst, [x | xs]} | stack], ret, out, cnt)
              x ->
                case [x | xs] do
                  [x] ->
                    next(f, line, [{:unary, x, []} | stack], ret, out, cnt)
                  xs ->
          ...
          {:get, [], ret, xs, ops} ->
            ...
            next(f, line, [{:get, [], {:get, type, ret, b}, xs, ops} | tl(stack)], ret, out, cnt)
          {:get, [], xs, ops} ->
            ["@" <> type | ops] = Enum.reverse(ops)
            type = tokenize_type(type)
            [a, b | xs] = (xs)
            next(f, line, [{:get, [], {:get, type, a, b}, xs, ops} | tl(stack)], ret, out, cnt)
          ...
          {:get, str, xs, ops} ->
            ...
            case get_part(str, ~r/@([^>].*|)/) do
              {x, op, rst} ->
                next(f, line, [{:get, rst, [x | xs], [op | ops]} | stack], ret, out, cnt)
              x ->
                case [x | xs] do
                  [x] ->
          ...
          {:mdr, [], ret, [], []} ->
            next(f, line, tl(stack), ret, out, cnt)
          {:mdr, [], ret, [x | xs], [op | ops]} ->
            next(f, line, [{:mdr, [], tk_mdr(op, ret, x), xs, ops} | tl(stack)], ret, out, cnt)
          {:mdr, [], xs, ops} ->
            [op | ops] = Enum.reverse(ops)
            [a, b | xs] = (xs)
            next(f, line, [{:mdr, [], tk_mdr(op, a, b), xs, ops} | tl(stack)], ret, out, cnt)
          {:mdr, [part | parts], xs, ops} ->
            if ret do
              next(f, line, [{:mdr, parts, [ret | xs], ops} | tl(stack)], nil, out, cnt)
            else
              next(f, line, [{:get, part, [], []} | stack], ret, out, cnt)
            end
          {:mdr, str, xs, ops} ->
            [_ | stack] = stack
            case get_part(str, ["*", "/", "%"]) do
              {x, op, rst} ->
                next(f, line, [{:mdr, rst, [x | xs], [op | ops]} | stack], ret, out, cnt)
              x ->
                case [x | xs] do
                  [x] ->
                    next(f, line, [{:get, x, [], []} | stack], ret, out, cnt)
                  xs ->
                    next(f, line, [{:mdr, xs, [], ops} | stack], ret, out, cnt)
                end
            end
          {:addsub, [], ret, [], []} ->
            next(f, line, tl(stack), ret, out, cnt)
          {:addsub, [], ret, [x | xs], [op | ops]} ->
            next(f, line, [{:addsub, [], tk_addsub(op, ret, x), xs, ops} | tl(stack)], ret, out, cnt)
          {:addsub, [], xs, ops} ->
            [op | ops] = Enum.reverse(ops)
            [a, b | xs] = (xs)
            next(f, line, [{:addsub, [], tk_addsub(op, a, b), xs, ops} | tl(stack)], ret, out, cnt)
          {:addsub, [part | parts], xs, ops} ->
            if ret do
              next(f, line, [{:addsub, parts, [ret | xs], ops} | tl(stack)], nil, out, cnt)
            else
              next(f, line, [{:mdr, part, [], []} | stack], ret, out, cnt)
            end
          {:addsub, str, xs, ops} ->
            [_ | stack] = stack
            case get_part(str, ["+", "-"]) do
              {x, op, rst} ->
                next(f, line, [{:addsub, rst, [x | xs], [op | ops]} | stack], ret, out, cnt)
              x ->
                case [x | xs] do
                  [x] ->
                    next(f, line, [{:mdr, x, [], []} | stack], ret, out, cnt)
                  xs ->
                    next(f, line, [{:addsub, xs, [], ops} | stack], ret, out, cnt)
                end
            end
          {:bshift, [], ret, [], []} ->
            next(f, line, tl(stack), ret, out, cnt)
          {:bshift, [], ret, [x | xs], [op | ops]} ->
            next(f, line, [{:bshift, [], tk_bshift(op, ret, x), xs, ops} | tl(stack)], ret, out, cnt)
          {:bshift, [], xs, ops} ->
            [op | ops] = Enum.reverse(ops)
            [a, b | xs] = (xs)
            next(f, line, [{:bshift, [], tk_bshift(op, a, b), xs, ops} | tl(stack)], ret, out, cnt)
          {:bshift, [part | parts], xs, ops} ->
            if ret do
              next(f, line, [{:bshift, parts, [ret | xs], ops} | tl(stack)], nil, out, cnt)
            else
              next(f, line, [{:addsub, part, [], []} | stack], ret, out, cnt)
            end
          {:bshift, str, xs, ops} ->
            [_ | stack] = stack
            case get_part(str, ["b<", "b>"]) do
              {x, op, rst} ->
                next(f, line, [{:bshift, rst, [x | xs], [op | ops]} | stack], ret, out, cnt)
              x ->
                case [x | xs] do
                  [x] ->
                    next(f, line, [{:addsub, x, [], []} | stack], ret, out, cnt)
                  xs ->
                    next(f, line, [{:bshift, xs, [], ops} | stack], ret, out, cnt)
                end
            end
          {:ineq, [], ret, [], []} ->
            next(f, line, tl(stack), ret, out, cnt)
          {:ineq, [], ret, [x | xs], [op | ops]} ->
            next(f, line, [{:ineq, [], tk_ineq(op, ret, x), xs, ops} | tl(stack)], ret, out, cnt)
          {:ineq, [], xs, ops} ->
            [op | ops] = Enum.reverse(ops)
            [a, b | xs] = (xs)
            next(f, line, [{:ineq, [], tk_ineq(op, a, b), xs, ops} | tl(stack)], ret, out, cnt)
          {:ineq, [part | parts], xs, ops} ->
            if ret do
              next(f, line, [{:ineq, parts, [ret | xs], ops} | tl(stack)], nil, out, cnt)
            else
              next(f, line, [{:bshift, part, [], []} | stack], ret, out, cnt)
            end
          {:ineq, str, xs, ops} ->
            [_ | stack] = stack
            case get_part(str, ["<", "<=", ">", ">="]) do
              {x, op, rst} ->
                next(f, line, [{:ineq, rst, [x | xs], [op | ops]} | stack], ret, out, cnt)
              x ->
                case [x | xs] do
                  [x] ->
                    next(f, line, [{:bshift, x, [], []} | stack], ret, out, cnt)
                  xs ->
                    next(f, line, [{:ineq, xs, [], ops} | stack], ret, out, cnt)
                end
            end
          {:eqneq, [], ret, [], []} ->
            next(f, line, tl(stack), ret, out, cnt)
          {:eqneq, [], ret, [x | xs], [op | ops]} ->
            next(f, line, [{:eqneq, [], tk_eqneq(op, ret, x), xs, ops} | tl(stack)], ret, out, cnt)
          {:eqneq, [], xs, ops} ->
            [op | ops] = Enum.reverse(ops)
            [a, b | xs] = (xs)
            next(f, line, [{:eqneq, [], tk_eqneq(op, a, b), xs, ops} | tl(stack)], ret, out, cnt)
          {:eqneq, [part | parts], xs, ops} ->
            if ret do
              next(f, line, [{:eqneq, parts, [ret | xs], ops} | tl(stack)], nil, out, cnt)
            else
              next(f, line, [{:ineq, part, [], []} | stack], ret, out, cnt)
            end
          {:eqneq, str, xs, ops} ->
            [_ | stack] = stack
            case get_part(str, ["=", "/="]) do
              {x, op, rst} ->
                next(f, line, [{:eqneq, rst, [x | xs], [op | ops]} | stack], ret, out, cnt)
              x ->
                case [x | xs] do
                  [x] ->
                    next(f, line, [{:ineq, x, [], []} | stack], ret, out, cnt)
                  xs ->
                    next(f, line, [{:eqneq, xs, [], ops} | stack], ret, out, cnt)
                end
            end
          {:band, [], ret, []} ->
            next(f, line, tl(stack), ret, out, cnt)
          {:band, [], ret, [x | xs]} ->
            next(f, line, [{:band, [], {:band, ret, x}, xs} | tl(stack)], ret, out, cnt)
          {:band, [], xs} ->
            [a, b | xs] = (xs)
            next(f, line, [{:band, [], {:band, a, b}, xs} | tl(stack)], ret, out, cnt)
          {:band, [part | parts], xs} ->
            if ret do
              next(f, line, [{:band, parts, [ret | xs]} | tl(stack)], nil, out, cnt)
            else
              next(f, line, [{:eqneq, part, [], []} | stack], ret, out, cnt)
            end
          {:band, str, xs} ->
            [_ | stack] = stack
            case get_part(str, ["b&"]) do
              {x, _, rst} ->
                next(f, line, [{:band, rst, [x | xs]} | stack], ret, out, cnt)
              x ->
                case [x | xs] do
                  [x] ->
                    next(f, line, [{:eqneq, x, [], []} | stack], ret, out, cnt)
                  xs ->
                    next(f, line, [{:band, xs, []} | stack], ret, out, cnt)
                end
            end
          {:biffxor, [], ret, [], []} ->
            next(f, line, tl(stack), ret, out, cnt)
          {:biffxor, [], ret, [x | xs], [op | ops]} ->
            next(f, line, [{:biffxor, [], tk_biffxor(op, ret, x), xs, ops} | tl(stack)], ret, out, cnt)
          {:biffxor, [], xs, ops} ->
            [op | ops] = Enum.reverse(ops)
            [a, b | xs] = (xs)
            next(f, line, [{:biffxor, [], tk_biffxor(op, a, b), xs, ops} | tl(stack)], ret, out, cnt)
          {:biffxor, [part | parts], xs, ops} ->
            if ret do
              next(f, line, [{:biffxor, parts, [ret | xs], ops} | tl(stack)], nil, out, cnt)
            else
              next(f, line, [{:band, part, []} | stack], ret, out, cnt)
            end
          {:biffxor, str, xs, ops} ->
            [_ | stack] = stack
            case get_part(str, ["b~", "b^"]) do
              {x, op, rst} ->
                next(f, line, [{:biffxor, rst, [x | xs], [op | ops]} | stack], ret, out, cnt)
              x ->
                case [x | xs] do
                  [x] ->
                    next(f, line, [{:band, x, []} | stack], ret, out, cnt)
                  xs ->
                    next(f, line, [{:biffxor, xs, [], ops} | stack], ret, out, cnt)
                end
            end
          {:bor, [], ret, []} ->
            next(f, line, tl(stack), ret, out, cnt)
          {:bor, [], ret, [x | xs]} ->
            next(f, line, [{:bor, [], {:bor, ret, x}, xs} | tl(stack)], ret, out, cnt)
          {:bor, [], xs} ->
            [a, b | xs] = (xs)
            next(f, line, [{:bor, [], {:bor, a, b}, xs} | tl(stack)], ret, out, cnt)
          {:bor, [part | parts], xs} ->
            if ret do
              next(f, line, [{:bor, parts, [ret | xs]} | tl(stack)], nil, out, cnt)
            else
              next(f, line, [{:biffxor, part, [], []} | stack], ret, out, cnt)
            end
          {:bor, str, xs} ->
            [_ | stack] = stack
            case get_part(str, ["b|"]) do
              {x, _, rst} ->
                next(f, line, [{:bor, rst, [x | xs]} | stack], ret, out, cnt)
              x ->
                case [x | xs] do
                  [x] ->
                    next(f, line, [{:biffxor, x, [], []} | stack], ret, out, cnt)
                  xs ->
                    next(f, line, [{:bor, xs, []} | stack], ret, out, cnt)
                end
            end
          {:and, [], ret, []} ->
            next(f, line, tl(stack), ret, out, cnt)
          {:and, [], ret, [x | xs]} ->
            next(f, line, [{:and, [], {:and, ret, x}, xs} | tl(stack)], ret, out, cnt)
          {:and, [], xs} ->
            [a, b | xs] = (xs)
            next(f, line, [{:and, [], {:and, a, b}, xs} | tl(stack)], ret, out, cnt)
          {:and, [part | parts], xs} ->
            if ret do
              next(f, line, [{:and, parts, [ret | xs]} | tl(stack)], nil, out, cnt)
            else
              next(f, line, [{:bor, part, []} | stack], ret, out, cnt)
            end
          {:and, str, xs} ->
            [_ | stack] = stack
            case get_part(str, ["&"]) do
              {x, _, rst} ->
                next(f, line, [{:and, rst, [x | xs]} | stack], ret, out, cnt)
              x ->
                case [x | xs] do
                  [x] ->
                    next(f, line, [{:bor, x, []} | stack], ret, out, cnt)
                  xs ->
                    next(f, line, [{:and, xs, []} | stack], ret, out, cnt)
                end
            end
          {:iffxor, [], ret, [], []} ->
            next(f, line, tl(stack), ret, out, cnt)
          {:iffxor, [], ret, [x | xs], [op | ops]} ->
            next(f, line, [{:iffxor, [], tk_iffxor(op, ret, x), xs, ops} | tl(stack)], ret, out, cnt)
          {:iffxor, [], xs, ops} ->
            [op | ops] = Enum.reverse(ops)
            [a, b | xs] = (xs)
            next(f, line, [{:iffxor, [], tk_iffxor(op, a, b), xs, ops} | tl(stack)], ret, out, cnt)
          {:iffxor, [part | parts], xs, ops} ->
            if ret do
              next(f, line, [{:iffxor, parts, [ret | xs], ops} | tl(stack)], nil, out, cnt)
            else
              next(f, line, [{:and, part, []} | stack], ret, out, cnt)
            end
          {:iffxor, str, xs, ops} ->
            [_ | stack] = stack
            case get_part(str, ["~", "^"]) do
              {x, op, rst} ->
                next(f, line, [{:iffxor, rst, [x | xs], [op | ops]} | stack], ret, out, cnt)
              x ->
                case [x | xs] do
                  [x] ->
                    next(f, line, [{:and, x, []} | stack], ret, out, cnt)
                  xs ->
                    next(f, line, [{:iffxor, xs, [], ops} | stack], ret, out, cnt)
                end
            end
          {:or, [], ret, []} ->
            next(f, line, tl(stack), ret, out, cnt)
          {:or, [], ret, [x | xs]} ->
            next(f, line, [{:or, [], {:or, ret, x}, xs} | tl(stack)], ret, out, cnt)
          {:or, [], xs} ->
            [a, b | xs] = (xs)
            next(f, line, [{:or, [], {:or, a, b}, xs} | tl(stack)], ret, out, cnt)
          {:or, [part | parts], xs} ->
            if ret do
              next(f, line, [{:or, parts, [ret | xs]} | tl(stack)], nil, out, cnt)
            else
              next(f, line, [{:iffxor, part, [], []} | stack], ret, out, cnt)
            end
          {:or, str, xs} ->
            [_ | stack] = stack
            case get_part(str, ["|"]) do
              {x, _, rst} ->
                next(f, line, [{:or, rst, [x | xs]} | stack], ret, out, cnt)
              x ->
                case [x | xs] do
                  [x] ->
                    next(f, line, [{:iffxor, x, [], []} | stack], ret, out, cnt)
                  xs ->
                    next(f, line, [{:or, xs, []} | stack], ret, out, cnt)
                end
            end
          {:assign, [], [a, b]} ->
            next(f, line, tl(stack), {:assign, a, b}, out, cnt)
          {:assign, [part | parts], xs} ->
            if ret do
              next(f, line, [{:assign, parts, [ret | xs]} | tl(stack)], nil, out, cnt)
            else
              next(f, line, [{:or, part, []} | stack], ret, out, cnt)
            end
          {:assign, str, xs} ->
            [_ | stack] = stack
            case get_part(str, ["<-"]) do
              {x, _, rst} ->
                next(f, line, [{:assign, rst, [x | xs]} | stack], ret, out, cnt)
              x ->
                case [x | xs] do
                  [x] ->
                    next(f, line, [{:or, x, []} | stack], ret, out, cnt)
                  [_, _] = xs ->
                    next(f, line, [{:assign, xs, []} | stack], ret, out, cnt)
                  _ ->
